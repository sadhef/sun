...
function CreateEnquiryModal({ isOpen, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (courses.length > 0) {
      // Initialize course rows with all courses
  ...
  const handleSubmit = async (statusType) => {
    ...
    if (clientType === 'company') {
      ...
      if (!selectedClient || !civilId) {
        toast.error('Please fill in all individual client details');
        return;
      }
    }
    ...
    try {
      ...
      // Create enquiries for each selected course
  ...
  return (
    <div className="fixed inset-0 z-[1001] flex items-center justify-center p-5 bg-black bg-opacity-50 opacity-100 pointer-events-auto">
      <div className="bg-white p-[25px] rounded-none max-h-[85vh] flex flex-col w-full max-w-[950px] relative animate-slideIn overflow-y-auto">
        <button
          onClick={onClose}
          className="text-[#aaa] absolute top-[15px] right-5 text-[28px] font-bold cursor-pointer hover:text-[#333]"
        >
          &times;
        </button>
        <div className="text-base font-bold m-0 mb-5 pr-5">Create New Enquiry</div>
        ...
        <div>
          ...
          <div className="bg-white p-[25px] rounded-lg shadow-[0_1px_3px_rgba(0,0,0,0.1)]">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse bg-white shadow-[0_1px_3px_rgba(0,0,0,0.1)]">
                <thead>
                  <tr>
                    <th className="p-[15px] text-left border-b border-border-color text-center">✓</th>
                    <th className="p-[15px] text-left border-b border-border-color">Course</th>
                    <th className="p-[15px] text-left border-b border-border-color text-center">Qty</th>
                    <th className="p-[15px] text-left border-b border-border-color">Training Date</th>
                    <th className="p-[15px] text-left border-b border-border-color text-center">Cost/Person</th>
                    <th className="p-[15px] text-left border-b border-border-color text-right">Total Cost</th>
                  </tr>
                </thead>
                <tbody>
                  {courseRows.map((row, index) => (
                    <tr key={index}>
                      <td className="p-2 text-left border-b border-border-color text-center">
                        <input
                          type="checkbox"
                      ...
                      <td className="p-2 text-left border-b border-border-color text-right font-bold">
          ...
          {/* Footer Actions */}
          <div className="mt-5 flex justify-end gap-[10px] flex-shrink-0">
            <button
              type="button"
              onClick={onClose}
              disabled={loading}
              className="px-5 py-[10px] border-none rounded-[5px] cursor-pointer text-sm font-bold transition-colors duration-200 bg-[#ccc] text-font-color hover:bg-opacity-90"
            >
              Cancel
            </button>
            <button
            ...
            <button
              onClick={() => handleSubmit('Quotation Sent')}
              disabled={loading}
            ...
            >
              {loading ? <i className="fas fa-spinner fa-spin mr-2"></i> : null}
              Generate Quotation <i className="fas fa-arrow-right ml-1"></i>
            </button>
          </div>
        </div>
      </div>
